...
/* ===================== PERF-UPGRADE: Parent → Iframe select via MessageChannel (fallback) ===================== */
/** Parent → Iframe selection; returns {changed, picked} — FAST (fallback, not used in optimistic path) */
...
// aggressive timings for snappier progress (safe with optimistic flow)
const T = { tick: 40, nextWait: 180, railTimeout: 1400, cardCleanTimeout: 900, errorsWaitMax: 2800, stuckSameSig: 1200, hardResetAfter: 2 };
...
/* ===================== FAST INDEX (IFRAME SIDE) — ultra mode ===================== */
(function initWidgetIndexUltra() {
    if (!IS_IFRAME) return;
    if (window.__WIDX__ && window.__WSEL__) return;

    const W = window.__WIDX__ = window.__WIDX__ || {
        version: 0,
        builtAt: 0,
        list: null,
    ...
    const stripBadgesFromLabel = (lab) => {
        if (!lab) return "";
    ...
    const kick = () => (window.requestIdleCallback ? requestIdleCallback(buildIndex, { timeout: 120 }) : setTimeout(buildIndex, 0));
    kick();

    const mo = new MutationObserver(() => {
        if (W._pending) return;
        W._pending = true;
        setTimeout(() => { W._pending = false; buildIndex(); window.__WSEL__?.reapply(); }, 0);
    });
    mo.observe(document.documentElement, { childList: true, subtree: true, attributes: true });

    // ===== Selection controller: persists last desired tokens & reapplies on DOM mutations
    window.__WSEL__ = {
        want: [], single: true,
        set(tokens = [], single = true) {
            this.want = Array.from(new Set((tokens || []).map(t => String(t).toLowerCase().trim()).filter(Boolean)));
            this.single = !!single;
            this.reapply();
        },
        reapply() {
            const list = W.getList?.();
            if (!list || !this.want.length) return false;

            // exact id/slug constant-time
            const exact = () => {
                for (const tok of this.want) {
                    if (W.mapById.has(tok)) return W.mapById.get(tok);
                    const id2 = W.slugToId.get(tok); if (id2) return W.mapById.get(id2);
                }
                return null;
            };
            ...
            if (!rec) {
                ...
                const iter = cands.size ? cands : W.mapById.keys();
                for (const id of iter) {
                    const r = W.mapById.get(id);
                    if (!r || !r.input || r.input.disabled) continue;
                    const cls = r.label?.className || "";
                    const unavailable = /\bline-through\b|\btext-muted\b|\bdisabled\b/.test(cls) ||
                        /\b0\s*available\b/i.test(r.label?.parentElement?.querySelector(".items-left, span.items-left")?.textContent || "");
                    if (unavailable) continue;
                    if (this.want.some(t => r.text.includes(t) || r.slug === t || id.toLowerCase() === t)) { rec = r; break; }
                }
            }
            if (!rec || !rec.input) return false;

            // ultra-fast: set state directly + batch one 'change' on list
            let changed = false;
            if (this.single) {
                const checked = list.querySelectorAll('input[type="checkbox"][id]:checked');
                for (const i of checked) { if (i !== rec.input) { i.checked = false; i.setAttribute('aria-checked', 'false'); changed = true; } }
            }
            if (!rec.input.checked) {
                rec.input.checked = true;
                rec.input.setAttribute('aria-checked', 'true');
                changed = true;
            }
            ...
            try { window.parent.postMessage({ type: 'JF_WIDGET_VALUE', values, value: values.join(', ') }, '*'); } catch { }
            try { window.parent.postMessage({ type: 'JF_WIDGET_VALUE_DIRTY' }, '*'); } catch { }
...
/** SINGLE: use controller; still returns {changed,picked} for compatibility */
function clickWidgetFirstAvailable(tokens = [], root = document) {
    const W = window.__WIDX__;
    if (!W) return { changed: false, picked: null };
    W.ensure();

    window.__WSEL__?.set(tokens, true);
    const list = W.getList?.();
    const checked = list?.querySelector('input[type="checkbox"][id]:checked');
    const picked = checked ? (checked.value || checked.id || null) : null;
    return { changed: !!picked, picked };
}
...
async function waitCardChange(oldId, { wait = 300 } = {}) { await delay(wait); const cur = getActiveCard(); return !!(cur && cur.id && cur.id !== oldId); }
...
/* ===================== IFRAME: ultra-fast handler using dedicated port (optimistic) ===================== */
if (IS_IFRAME && !window.__JF_IFRAME_FAST_SELECT__) {
    ...
    window.addEventListener("message", (ev) => {
        ...
        // Persist desired tokens and apply asap; reply immediately for optimistic UX
        window.__WSEL__?.set(data.tokens || [], !!data.single);
        try { port.postMessage({ type: "JF_WIDGET_SELECTED", changed: true, picked: null, values: [] }); } catch { }
    }, false);
}
...
/* ===================== Optimistic pick + NEXT (parent) ===================== */
async function optimisticPickAndNext(card, tokensForWidget) {
    const comps = getWidgetComponents(card);
    for (const comp of comps) {
        const iframe = findWidgetIframeInComp(comp);
        if (!iframe) continue;
        const origin = (() => { try { return new URL(iframe.src).origin; } catch { return "*"; } })();
        const ch = new MessageChannel();
        try {
            iframe.contentWindow.postMessage(
                { type: "JF_WIDGET_SELECT_FAST", tokens: tokensForWidget, single: true },
                origin,
                [ch.port2]
            );
            setTimeout(() => { try { ch.port1.close(); } catch { } }, 0);
        } catch { }
    }

    // Optimistically unlock NEXT immediately
    const next = getNextBtn(card);
    if (next) {
        next.disabled = false; next.removeAttribute("disabled"); next.removeAttribute("aria-disabled");
        next.classList.remove("disabled", "isDisabled"); next.style.pointerEvents = "";
        next.scrollIntoView({ block: "center" });
        await new Promise(r => requestAnimationFrame(r)); // let iframe apply state
        next.click();
    }
}
...
async function rescueCurrentCard(
    ...
    if (hasWidgetInCard(card) && shouldTickCard(card, enabledDaysSet, includeSpecialEvent)) {
        await optimisticPickAndNext(card, tokensForWidget);
        await waitCardCleanFast(card, { timeout: T.cardCleanTimeout });
...
async function waitCardCleanFast(card, { timeout = 900 } = {}) { const ok = () => !(card.querySelector('li.form-line-error, .form-line.form-validation-error, li[aria-invalid="true"]') || card.classList.contains('animate-shake') || card.querySelector('.jfCard.animate-shake')); const res = await waitWithObserver(card, { predicate: ok, timeout }); if (!res) return ok(); await nextFrame(); return true; }
async function waitRailClearedFast(qid, { timeout = 1400 } = {}) { const lbl = qs(`#cardProgress .jfProgress-itemLabel[data-item-id="${qid}"]`); const item = lbl?.closest('.jfProgress-item'); const ok = () => !railHasError(qid); const res = await waitWithObserver(item || document.body, { predicate: ok, timeout }); if (!res) return ok(); await nextFrame(); return true; }
...
async function resolveErrorsOnCard(
    ...
    if (hasWidgetInCard(card) && (hasLineErrorInCard(card) || railHas)) {
        ...
        if (shouldTickCard(card, enabledDaysSet, includeSpecialEvent)) {
            // in error path, we can still use fast optimistic pick
            await optimisticPickAndNext(card, tokensForWidget);
            await waitCardCleanFast(card, { timeout: T.cardCleanTimeout });
            await waitRailClearedFast(qid, { timeout: T.railTimeout });
        }
    ...
    try {
        ...
        for (let loop = 0; loop < maxLoops; loop++) {
            ...
            for (const qid of qids) {
                ...
                await waitRailClearedFast(qid, { timeout: 1400 });
                const cur = getActiveCard(); if (cur && cardIdToQid(cur) === qid) { await waitCardCleanFast(cur, { timeout: 900 }); }
                await nextFrame();
            }
...
async function mainLoop(payload) {
    ...
    while (window.isFilling) {
        ...
        // ==== Widget select — SINGLE by priority (only once per eligible card) — OPTIMISTIC
        if (tokensForWidget.length && hasWidgetInCard(card) && !widgetSentForCard.has(card.id)) {
            if (!shouldTickCard(card, enabledDaysSet, includeSpecialEvent)) {
                ...
                await optimisticPickAndNext(card, tokensForWidget);
                widgetSentForCard.add(card.id);
                // no extra nudge; NEXT already clicked optimistically
            }
        }
